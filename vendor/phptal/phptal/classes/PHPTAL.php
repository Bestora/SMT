...
class PHPTAL
{
  ...
  /**
   * Prefilters have been redesigned.
   * Old property is no longer used.
   *
   * @deprecated
   *
   */
   ...
   *
   * @param string $src
   *            The phptal template source.
   * @param string $path
   *            Fake and 'unique' template path.
   *
   * @return $this
   */
   ...
   *
   * @param mixed $rep
   *            string or Array of repositories
   *
   * @return $this
   */
   ...
   *
   * @param bool $bool
   *            if true all comments are stripped during parse
   *
   * @return $this
   */
   ...
   *
   * @param int $mode
   *            (PHPTAL::XML, PHPTAL::XHTML or PHPTAL::HTML5).
   *
   * @return $this
   */
   ...
   *
   * @param PHPTAL_TranslationService $t
   *            instance
   *
   * @return $this
   */
   ...
   *
   * @param mixed $filter
   *            PHPTAL_PreFilter object or name of prefilter to add
   *
   * @return PHPTAL
   */
  ...
  public function execute()
  {
    ...
    try {
      ...
      $error = new SMTError('TPL_Error.log', $e);
      ob_end_clean();
      throw $e;
    }
   ...
   *
   * @param PHPTAL $local_tpl
   *            is PHPTAL instance of the file in which macro is defined
  ...
  public function getFunctionName()
  {
    ...
    if (!$this->_functionName) {
      ...
      $hash = md5(PHPTAL_VERSION . PHP_VERSION . $this->_source->getRealPath() . $this->getEncoding() . $this->getPrefiltersCacheId() . $this->getOutputMode(), true);
      ...
      $this->_functionName = $this->getFunctionNamePrefix($this->_source->getLastModifiedTime()) . $basename . '__' . $hash;
    }
   ...
   *
   * @param int $timestamp
   *            unix timestamp with template modification date
   *
   * @return string
   */
